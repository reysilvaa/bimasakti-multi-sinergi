...
class PdamApp {
    ...
    try {
      ...
      for (const p of json.data) {
        ...
        if (presetWrap) {
          ...
          btn.type = "button";
          btn.className =
            "px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors flex items-center gap-1.5";
  ...
  private switchTab(tab: "inquiry" | "history"): void {
    ...
    const activeClass =
      "px-4 py-2 rounded-md text-sm font-medium transition-all bg-blue-600 text-white shadow-sm flex items-center gap-2";
    const inactiveClass =
      "px-4 py-2 rounded-md text-sm font-medium transition-all text-blue-200 hover:text-white hover:bg-blue-800/50 flex items-center gap-2";
  ...
  private async handlePaymentSubmit(): Promise<void> {
    ...
    this.btnProceedPayment.innerHTML =
      '<i class="fa-solid fa-spinner animate-spin"></i><span>Memproses Pembayaran...</span>';
    ...
    try {
      ...
      this.btnProceedPayment.innerHTML =
        '<i class="fa-solid fa-credit-card"></i><span>Konfirmasi & Bayar Sekarang</span>';
    }
  }
